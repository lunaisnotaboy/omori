...
function patch() {
  ...
  const lbase = base + "/languages"
  function patchName(name: string) {
    if (name.startsWith(base)) {
      const path = base + name.slice(base.length).toLowerCase()
      if (path.startsWith(lbase)) {
        return lbase + name.slice(lbase.length)
      }
      return path
    }
    return name
  }
  for (const fn of ["readFile", "readFileSync", "readdirSync", "statSync", "writeFile"]) {
    const real = fs[fn]
    fs[fn] = function (name: string, ...rest: any) {
      if (fn === "writeFile" && typeof rest[0] === "number") {
        (rest as any)[0] += ""
      }
      let ret = real.call(this, patchName(name), ...rest)
      return ret
    }
  }
  if (process.platform === "linux") {
    process.env.LOCALAPPDATA = (process.env.XDG_DATA_HOME || (process.env.HOME + "/.local/share"))
  }
  const realImage = Image;
  window.Image = class Image extends realImage {
    get src() {
      return this.getAttribute("src") || ""
    }
    set src(value) {
      this.setAttribute("src", value.toLowerCase())
    }
  }
  const realXhrOpen = XMLHttpRequest.prototype.open
  XMLHttpRequest.prototype.open = function (this: XMLHttpRequest, ...args: Parameters<typeof realXhrOpen>) {
    args[1] = args[1].toString().toLowerCase()
    return realXhrOpen.call(this, ...args)
  } as typeof realXhrOpen
  Object.defineProperty(window.nw.App, "argv", { value: ["6bdb2e585882fbd48826ef9cffd4c511"] })
}
patch()
...
ConfigManager.applyData = function (config) {
  ...
  // this prop is force set to true later and trying to change it crashes the game
